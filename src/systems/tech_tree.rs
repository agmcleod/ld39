        ...
        if let Some(tech_tree_node_entity) = tech_tree_node_entity {
            ...
            if create_tooltip {
                if let Some(container_node) = scene.get_node_for_entity(*lookup.entities.get(&"tech_tree_container".to_string()).unwrap()) {
                    let tooltip_node = create_tooltip::create(&entities, &mut color_storage, &mut rect_storage, &mut text_storage, &mut transform_storage, tooltip_position[0], tooltip_position[1]);
