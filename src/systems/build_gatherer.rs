...
use components::{AnimationSheet, Button, Gatherer, Input, Resources, SelectedTile, Transform};
...
impl<'a> System<'a> for BuildGatherer {
    ...
    fn run(&mut self, data: Self::SystemData) {
        let (mut animation_sheet_storage, mut button_storage, entities, mut gatherer_storage, input_storage, mut resources_storage, mut selected_tile_storage, mut transform_storage) = data;
        ...
        for (selected_tile, transform) in (&mut selected_tile_storage, &transform_storage).join() {
            if button_pressed && selected_tile.visible && resources.get_resources_to_buy(20) > 0 {
