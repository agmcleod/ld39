...
pub fn create(
    ...
    let alpha = if mode == FadeMode::In { 0.0 } else { 1.0 };

    color_storage
        .insert(entity, Color([1.0, 1.0, 1.0, alpha]))
        .unwrap();
    rect_storage.insert(entity, Rect {}).unwrap();
    fade_storage
        .insert(entity, Fade::new(mode, duration))
        .unwrap();
    ...
    transform_storage
        .insert(
            entity,
            Transform::visible(0.0, 0.0, 50.0, dim[0] as u16, dim[1] as u16, 0.0, 1.0, 1.0),
        )
        .unwrap();
    ...
    if let Some(transition_to_state_storage) = transition_to_state_storage {
        transition_to_state_storage
            .insert(
                entity,
                TransitionToState::new(state_to_transition_to.unwrap()),
            )
            .unwrap();
    }
